...
async function load_chat(userName) {
        ...
        if (!getSecondaryKey()) {
            ...
            await sym.importKey(asymmetric_private_instance);
            await generate_room_private_keys();
        }
        ...
        await load_chat_message(messages, '{{ room_data.keys.public_keys }}');
        ...
        $.ajax({
            ...
            success: async function (response) {
                if (response.status === 'success') {
                    ...
                    console.error('Error loading messages:', response.error);
                    reject(response.error); // Reject the Promise if the AJAX request is unsuccessful
                }
            },
            error: function (error) {
                console.error('Error loading messages:', error);
...
function initWebSocket(web_socket_url) {
    return new Promise((resolve, reject) => {
        const chatSocket = new WebSocket(web_socket_url);
...
async function initialize_socket() {
    ...
    try {
        chatSocket = await initWebSocket(web_socket_url);
    } catch (error) {
        ...
        if (message !== '' || imageInput.files.length > 0) {
            ...
            if (imageInput.files.length > 0) {
                ...
                if (containsSnapMessage) {
                    ...
                    let encodedData = msgpack.encode(data);
...
async function generate_room_private_keys() {
    return new Promise((resolve, reject) => {
        let roomData = getKeysByRoom(getActiveRoom());
